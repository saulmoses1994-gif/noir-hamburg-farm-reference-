...
export async function buildMetadata({ title, description, image, imageAlt, path, lang = 'de' }) {
  // Fall back to the site-wide social share image (settings.social_share_image)
  // when a page doesn't pass an explicit image. This is what WhatsApp,
  // Facebook, LinkedIn, iMessage, Slack, etc. pick up for link previews.
  ...
  if (!resolvedImage) {
    ...
    } catch { /* keep resolvedImage as-is */ }
  }
  return {
