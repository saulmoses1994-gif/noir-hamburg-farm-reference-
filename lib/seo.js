...
// SEMrush flags it as a duplicate. We defend by measuring how many
// significant words the authored meta actually shares with the article's
// real title — if the overlap is low, the meta is treated as unrelated and
// we fall back to `${articleTitle} | Noir Hamburg`.
//
// Why word-overlap ratio (and not just a prefix check):
//   • A prefix check (e.g. "does the meta start with the first N chars of
//     the title") produces false positives when both texts happen to begin
//     with the same domain word — which is exactly what tripped the earlier
//     8-char version. Example that fooled the prefix check:
//       article: "Diskretion im Zeitalter digitaler Spuren — Wie wir …"
//       meta:    "Diskretion & Datenschutz | Noir Hamburg Premium Escort"
//     Both start with "Diskreti…" but they're about very different topics.
//   • Word-overlap counts EVERY meaningful word (>=5 chars). A correctly
//     authored meta shares nearly all article words with the title
//     (ratio ~1.0). A mistakenly pasted foreign meta shares at most one
//     shared theme word (ratio typically < 0.2).
//   • Threshold set to 0.35 (35%) — low enough that lightly re-worded metas
//     stay accepted, high enough that clear collisions get rejected.
export function resolveArticleTitle(articleTitle, authoredMeta, brand = 'Noir Hamburg') {
  ...
  // Extract significant words (length ≥5, lowercase, Unicode-safe for
  // German umlauts). Any punctuation and separator character is treated
  // as a boundary.
  const words = (s) =>
    (s.toLowerCase().match(/[\p{L}\d]+/gu) || []).filter((w) => w.length >= 5)
  const articleWords = words(article)
  if (articleWords.length === 0) return meta // too short to reason about
  const metaWords = new Set(words(meta))
  const overlap = articleWords.filter((w) => metaWords.has(w)).length
  const ratio = overlap / articleWords.length
  return ratio >= 0.35 ? meta : fallback
}
...
export async function buildMetadata({
  title, description, image, imageAlt, path, lang = 'de',
  hasEnAlternate = true, hasDeAlternate = true, noindex = false,
  enPath, dePath,
  // Optional OG overrides — when set they replace the general
  // title/description ONLY inside og: (and twitter:) tags. `<title>` and
  // `<meta name="description">` still use the SEO title/description so
  // search engines and social previews can diverge intentionally.
  ogTitle,
  ogDescription,
  // Article-specific fields (only used when ogType === 'article').
  // These are surfaced through Next.js's metadata API which then emits:
  ...
  // Callers on non-article pages omit `ogType` entirely and get the
  // default `type: 'website'` which is correct for the homepage, service
  // pages, contact, model pages, category / archive pages, etc.
  ogType = 'website',
  publishedTime,
  modifiedTime,
  authors,
  ...
  //
  // MULTILINGUAL BLOG: when the DE and EN paths differ (blog articles get
  // an auto-derived slug_en that is distinct from the DE slug), callers
  // pass explicit dePath/enPath so we emit the correct hreflang pair
  // instead of relying on the (blog-agnostic) SLUG_MAP.
  let alternates
  if (noindex) {
    alternates = { canonical: `${SITE_URL}${lang === 'en' ? '/en' : ''}${path === '/' ? '' : path}` }
  } else if (dePath || enPath) {
    const deAbs = `${SITE_URL}${dePath || path}`
    const enAbs = `${SITE_URL}${enPath || (lang === 'en' ? `/en${path === '/' ? '' : path}` : `/en${path === '/' ? '' : path}`)}`
    const canonicalAbs = lang === 'en' ? enAbs : deAbs
    const languages = {}
    if (lang === 'de') languages.de = deAbs
    else languages.en = enAbs
    ...
    alternates = alternatesForLang(lang, path, { hasEnAlternate, hasDeAlternate })
  }
  const openGraph = {
    title: ogTitle || title,
    description: ogDescription || description,
    url: alternates.canonical || `${SITE_URL}${lang === 'en' ? '/en' : ''}${path === '/' ? '' : path}`,
  ...
  if (ogType === 'article') {
    ...
    // Next.js requires OG authors as an ARRAY of strings (or URLs).
    if (authors) openGraph.authors = Array.isArray(authors) ? authors : [authors]
    if (section) openGraph.section = section
    if (Array.isArray(tags) && tags.length) openGraph.tags = tags
  }
  return {
