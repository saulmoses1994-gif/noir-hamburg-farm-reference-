...
export function ensureFormattedHtml(body) {
  ...
  // Still sanitise so raw pastes with unsafe tags/attrs are neutralised.
  if (BLOCK_TAG_RE.test(body)) return sanitizeHtml(body)
  // Plain-text / Markdown path — marked handles headings (## ###),
  // ordered/unordered lists, bold, italic, links, blockquotes, tables,
  // and code blocks. We ALWAYS run the sanitiser on the resulting HTML
  // so raw `<script>` snippets that slipped past the write path (or
  // pre-existing DB content) can never reach the browser.
  try {
    return sanitizeHtml(marked.parse(body).trim())
  } catch {
    return `<p>${body}</p>`
  }
}
