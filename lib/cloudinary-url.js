...
export function optimizeImageUrl(url, opts = {}) {
  ...
  const {
    w, h, ar, crop, gravity = 'auto',
    // Quality controls (default `auto` = balanced; `auto:best` picks a higher
    // target quality for hero-level assets; explicit numbers 1-100 also work).
    quality,
    // Sharpening (1-2000). ~60-100 = subtle edge sharpening that hides slight
    // upscale softness without introducing halos.
    sharpen,
    // Cloudinary AI enhancement filters
    improve = false,
  } = opts
  const parts = ['f_auto']
  parts.push(quality ? `q_${quality}` : 'q_auto')
  ...
  if (sharpen) parts.push(`e_sharpen:${sharpen}`)
  parts.push('dpr_auto')
  const tx = parts.join(',')
