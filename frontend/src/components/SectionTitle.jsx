export default function SectionTitle({ overline, title, description, align = "left" }) {
  ...
  return (
    <div className={`flex flex-col gap-4 max-w-3xl ${alignment}`} data-testid="section-title">
      ...
      <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-light tracking-tight leading-tight text-[#F5F5F0]">
        {title}
      </h2>
      {description && (
        <p className="text-base lg:text-lg font-light text-[#9CA3AF] leading-relaxed max-w-2xl">
          {description}
        </p>
      )}
