        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/5">
          {SERVICES.map((s, i) => (
            <Link
              key={s.slug}
              to={`/services/${s.slug}`}
              className="bg-[#0A0A0B] hover:bg-[#121214] transition-colors duration-500 group block relative overflow-hidden"
            ...
            >
              ...
              <div className="p-8 lg:p-12">
                ...
                <p className="font-heading italic text-[#9CA3AF] mt-1">{s.tagline}</p>
                <p className="mt-5 text-sm font-light text-[#9CA3AF] leading-relaxed">{s.description}</p>
...
export function ServiceDetail() {
  ...
  if (!service) {
    return <PublicLayout><div className="px-6 py-32 text-center text-[#9CA3AF]">Service nicht gefunden.</div></PublicLayout>;
  }
    ...
    <PublicLayout>
      <section className="relative h-[60vh] flex items-end" data-testid="service-hero">
        ...
        <div className="relative z-10 px-6 md:px-12 lg:px-16 pb-12 max-w-4xl">
          ...
          <span className="overline block mt-6 mb-4">{service.shortLabel}</span>
          <h1 className="font-heading text-5xl lg:text-7xl font-light tracking-tighter leading-none">{service.h1}</h1>
          <p className="font-heading italic text-xl text-[#9CA3AF] mt-4">{service.tagline}</p>
        </div>
      </section>
      ...
      <section className="px-6 md:px-12 lg:px-16 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7">
            ...
            <p className="text-base lg:text-lg font-light text-[#9CA3AF] leading-relaxed">{service.longCopy}</p>

            <div className="mt-16">
              <span className="overline">Charakteristika</span>
              <ul className="mt-6 space-y-4">
                {service.keypoints.map((k) => (
                  <li key={k} className="flex gap-4 items-start border-t border-white/5 pt-4">
                    <span className="accent-text font-mono text-xs mt-1">·</span>
                    <span className="text-base font-light text-[#F5F5F0]">{k}</span>
                  </li>
                ))}
              </ul>
            </div>
