...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    title: pick(s, 'meta_title', 'en') || s.title,
    description: pick(s, 'meta_description', 'en') || pick(s, 'description', 'en'),
    image: s.image,
    imageAlt: pick(s, 'image_alt', 'en') || s.title,
    path: `/services/${slug}`,
    lang: 'en',
  })
}
      ...
      <main id="main">
        ...
        <section className="relative h-[60vh] flex items-end">
          ...
          <div className="relative z-10 px-6 md:px-12 lg:px-16 pb-12 max-w-4xl text-white">
            <Breadcrumbs items={[{ label: 'Services', href: '/en/services' }, { label: pick(s, 'title', 'en') || s.title }]} dark />
            <span className="overline block mt-6 mb-4 text-[#E5A5B5]">{pick(s, 'short_label', 'en') || s.short_label}</span>
            <h1 className="font-heading text-5xl lg:text-7xl font-semibold tracking-tight leading-tight text-white">{pick(s, 'title', 'en') || s.h1 || s.title}</h1>
        ...
        <section className="px-6 md:px-12 lg:px-16 py-24">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8">
              ...
              {keypoints.length > 0 && (
                <div className="mt-16">
                  <span className="overline">Characteristics</span>
              ...
              {faqs.length > 0 && (
                <div className="mt-16">
                  ...
                  <h2 className="font-heading text-2xl lg:text-3xl text-[#1A1414] mt-3 mb-6">FAQ — {s.title}</h2>
