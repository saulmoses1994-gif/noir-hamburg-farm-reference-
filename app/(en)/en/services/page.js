...
export async function generateMetadata() {
  return buildMetadata({
    title: 'Escort Services Hamburg — Premium Companionship | Noir Hamburg',
    description: 'Eight carefully defined service categories from Noir Hamburg: Luxury, VIP, Business, Dinner, Hotel, Event, Travel and Girlfriend Experience.',
    path: '/services',
    lang: 'en',
  })
}
...
export default async function ServicesListEn() {
  ...
  const jsonLd = [
    breadcrumbSchema([{ name: 'Home', url: '/en' }, { name: 'Services', url: '/en/services' }]),
        ...
        <section className="px-6 md:px-12 lg:px-16 pb-32">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[#1A1414]/5">
            {services.map((s, i) => {
              ...
              return (
                <Link key={s.slug} href={`/en/services/${s.slug}`} className="bg-white hover:bg-[#FBF7F4] transition-colors duration-500 group block relative overflow-hidden">
