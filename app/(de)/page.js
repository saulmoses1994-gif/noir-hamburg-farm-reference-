...
export default async function Home() {
  ...
  const jsonLd = [
    ...
    { '@context': 'https://schema.org', '@type': 'WebSite', name: 'Noir Hamburg', url: siteUrl(), inLanguage: 'de-DE' },
  ]
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center max-w-7xl">
            <div className="lg:col-span-7 order-1">
              ...
              <div className="mt-10 flex gap-4 flex-wrap">
                <Link href="/services" className="btn-primary">Services entdecken</Link>
                <Link href="/models" className="btn-ghost">Models</Link>
              </div>
            </div>
            {hero && (
        ...
        <section className="px-6 md:px-12 lg:px-16 py-16 border-t border-[#1A1414]/8">
          <div className="flex items-baseline justify-between mb-10">
            <div>
              ...
              <h2 className="font-heading text-4xl lg:text-5xl mt-3">Unsere Escort Services</h2>
            </div>
            <Link href="/services" className="text-xs font-mono uppercase tracking-[0.2em] accent-text hover:opacity-70">Alle ansehen →</Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#1A1414]/5">
            {services.map((s) => (
              <Link key={s.slug} href={`/services/${s.slug}`} className="bg-white hover:bg-[#FBF7F4] transition-colors p-8 block group">
